...
router.get("/managerDash", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  if (req.user && (req.user.role !== "manager" && req.user.role !== "director")) {
  ...
  try {
    ...
    const adjustmentHistory = await StockAdjustment.find({}).populate('product').sort({ adjustmentDate: -1 }).limit(5); // Populate the 'product' field
...
router.post('/api/bulkAdjust', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  if (req.user && (req.user.role !== "manager" && req.user.role !== "director")) {
  ...
  try {
    ...
    //update and save
    productToAdjust.kilos += parseFloat(kilos); // Ensure kilos is treated as a number
    ...
    const newAdjustmentLog = new StockAdjustment({
      product: productToAdjust._id, // Use the ObjectId of the procurement document
      quantityAdjusted: parseFloat(kilos),
      reason: reason,
      notes: notes,
...
router.put('/api/adjustStock', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  if (req.user && (req.user.role !== "manager" && req.user.role !== "director")) {
    return res.status(403).send("Access denied: You are not authorized to access this resource.");
  }
  try {
    ...
    productToAdjust.kilos = parseFloat(newStock); //update happens here.
